...
class main():
    ...
    def __init__(self, path, matching_database, lcia_meth='recipe', bw_project="Single Use vs Multi Use"):
        """
    ...
    def is_valid_flow(self, temp, flow):
        return (('H2' in temp or 'H4' in temp) and ('SU' in temp or 'REC' in temp) and temp not in flow)

    def get_all_flows(self, path, lcia_meth='recipe', case_range=range(1, 3)):
        ...
        for nr in case_range:
            ...
            for tp in db_type:
                ...
                self.database_name_dct[database_name] = database_name
                ...
                if 'case1' in str(db):
                    for act in db:
                        ...
                        if self.is_valid_flow(temp, flow):
                            flow.append(temp)
                    ...
                    self.sheet_name[database_name] = 'case1'
                # Check if the database is case2
                    ...
                    for act in db:
                        ...
                        if temp == 'SUD' or temp == 'MUD':
                            flow.append(temp)
                        flow_leg = ['SUD', 'MUD']
                        self.sheet_name[database_name] = 'case2'
                    flow.sort()
                    flow.reverse()

                # Store the flow and other information in the respective dictionaries
                self.flows[database_name] = flow
                dir_temp = self.results_folder(self.join_path(path, 'results'), f"case{nr}")
                self.save_dir[database_name] = dir_temp
                self.file_name[database_name] = self.join_path(dir_temp, f"data_case{nr}_{tp}_recipe.xlsx")
                self.db_type_dct[database_name] = tp
                self.flow_legend[database_name] = flow_leg
                self.file_name_unique_process[database_name] = self.join_path(dir_temp, f"data_uniquie_case{nr}_{tp}_{lcia_meth}.xlsx")
                self.initialization[database_name] = [self.bw_project, database_name, flow, lcia_meth, tp]

        # Create a list of the collected information
        lst = [self.save_dir, self.file_name, self.flow_legend, self.file_name_unique_process, self.sheet_name]
        
        return lst, self.initialization
    ...
    def data_paths(self):
        ...
        main_folder_path = self.join_path(self.path, r'RA\Single-use-vs-multi-use-in-health-care')
        ...
        results_path = [
            self.join_path(self.path_github, r'Data\databases\case1.xlsx'), 
            self.join_path(self.path_github, r"Data\databases\case2.xlsx")
                ]
    ...
    def reload_database(self, proj_database, sheet, reload):
        ...
        if reload:
            if isinstance(self.system_path, list):
                for spath in self.system_path:
                    data = pd.read_excel(spath, sheet_name=sheet)
            ...
            elif isinstance(self.system_path, str):
                ...
                print(f"Wrong data format for {self.system_path} - {type(self.system_path)}")
